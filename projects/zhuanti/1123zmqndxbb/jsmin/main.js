        ...
        function addSource(sid, sty) {
            ...
            switch (sty) {
                ...
                case 'video_yk':
                    ...
                    break;
                case 'video_yk2':
                    sourceDom = '<iframe height="100%" width="100%" src="http://player.youku.com/embed/' + sid + '==" frameborder=0 allowfullscreen></iframe>';
                    return sourceDom;
                    break;
                case 'video_other':
                    sourceDom = sid;
...
(function ($) {
    var ymjsModel = {
        ...
        ztSld: function ztSld(sldId) {
            var $sldId = $(sldId),
                $sldNav = $sldId.find('.ztSldNav').find('li'),
            ...
            $(sldId).slide({
                ...
                endFun: function endFun(i, c) {
                    $sldCon.find('li').removeClass('cur').eq(i + 1).addClass('cur');
                },
                startFun: function startFun(i, c) {
                    $sldCon.find('li').removeClass('cur');
                }
            });
            $sldId.find('.btn').on('click', function () {
                var $this = $(this),
                    $par = $this.closest('li');
                $par.find('.rwCell').removeClass('cur');
                $par.find('.btn').removeClass('cur');
                $this.addClass('cur');
                if ($this.hasClass('btnBoy') === true) {
                    $par.find('.rwPicBoy').addClass('cur');
                } else if ($this.hasClass('btnGirl') === true) {
                    $par.find('.rwPicGirl').addClass('cur');
                }
            });
        },
        //右侧导航按钮
    ...
    ymjsModel.ztSld('#sld5');
    ymjsModel.fnavA('#fixednav', 800, 1330);
})(jQuery);