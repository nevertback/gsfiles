...
(function ($) {
	var ymjsModel = {
        ...
        s3tavinner:function(){
            var $tab = $('.ztTabInner'),
                $nav = $tab.find('.ztTabInnerNav'),
                $con = $tab.find('.ztTabInnerCon'),tabTimer;
            $nav.find('a').on({
                mouseover:function () {
                    var $ts = $(this),idx = $ts.index();
                    tabTimer = setTimeout(function () {
                        $con.find('.ztTabInnerConItem').removeClass('cur').eq(idx).addClass('cur');
	...
	ymjsModel.s3tavinner();
	ymjsModel.fnavA('#fixednav',100,1320);
})(jQuery);