	...
	var ymjsModel = {
        ...
        smtFunc:function () {
            ...
            $('.smtOpen').on('click',function () {
                var $this = $(this),txt = $this.data('txt');
                $('.smtPop1').addClass('cur').find('h5').html(txt);
            });
            $('.btnUpd').on('click',function () {
                $('.smtPop2').addClass('cur');
            });
            $('.smtPopClose').on('click',function () {
                $('.smtPop').removeClass('cur');
            });
            $('#smtBtn').on('click', function() {
                var Ml = 'poebxh',Folder = 'zhuanti/'+Ml+'/',fname = 'infos',
                ...
                var content = "---email：" + userNum + "---礼包类型：" + userCon + "---";
                $.ajax({
                    type: "GET",
                    dataType: "jsonp",
