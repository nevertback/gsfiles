(function(a) {
    ...
    var i = function() {
        ...
        if (f.showPercentage) {
            ...
            $('#loadingAnim').find('.loadingtxt').text("");
        }
        if (!f.autoclose) {
    ...
    var m = function() {
        ...
        $('#loadingAnim').find('.loadingAnim-img').stop().animate({width: (100 - c) + "%"}, 500, "linear");
        if (f.showPercentage) {
            a(jPer).text(c + "%");
            $('#loadingAnim').find('.loadingtxt').text(c + "%");
        }
        if (e >= b.length) {
            e = b.length;
            h();
            if (f.showPercentage) {
                a(jPer).text("100%")
                $('#loadingAnim').find('.loadingtxt').text("100%");
            }
            if (f.debugMode) {
                var d = o()
            }
