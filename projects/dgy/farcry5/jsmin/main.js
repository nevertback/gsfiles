...
var pageFunc = {
	loadingFunc:function(loadState){
    	...
    	ld += '<p><span>为了更好的观看体验，请使用<i>IE10以上</i>或其他浏览器浏览专题。</span></p>';
    	ld += '</div>';
    ...
    pgMenu:function(){
        ...
        $.each(dataOrigin.nav,function (i,item) {
            ...
            btnDom += '<a class="btn-sel btnSel btnSel'+i+' '+isCur+'" data-idx="'+i+'"><i class="icon"></i>'+item.name+'</a>';
            introDom += '<div class="task-intro taskIntro taskIntro'+i+'"><img src="'+item.tro+'" alt="'+item.name+'">';
            introDom += '<a class="begin-task beginTask" data-idx="'+i+'">';
            if(item.open === true){
                introDom += '重玩任务';
            }else{
                introDom += '开始任务';
        ...
        function thisFunc(){
            ...
            dgyMain.find('.beginTask').on('click',function(){
                var $ts = $(this),idx=$ts.data('idx');
                pageFlow['task'+(idx+1)]();
            });
            ...
            dgyMain.find('.btnSel'+stateStore.taskProgress).addClass('cur');
            dgyMain.find('.taskIntro'+stateStore.taskProgress).addClass('cur');
        }
        yms.chagePage(function(){
            stateStore.fixedNavShow = true;
            dgyMain.html(pageDom).addClass('cur');
    ...
    pgVersion:function(back){
        ...
        pageDom += '</div>';
        pageDom += '<a class="btn-default btn-next btnNext">';
        ...
        function thisFunc(){
            ...
            var setCon = {
                ...
                tab1:function (idx) {
                    var tabDom = '',dt = dataOrigin.tabs[idx];
                    tabDom += '<div class="tabs-con-in tabs-con'+idx+'">';
                    ...
                    commonComponents.mouseScrollTips($con);
