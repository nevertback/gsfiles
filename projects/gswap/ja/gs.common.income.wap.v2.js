(function ($) {
    ...
    gsIncomeWap.prototype.lkV2=function (options) {
        ...
        //tgDom += '<div style="background-image: url('+tgImg+');" class="gsTgWapLkInnerMain">';
        tgDom += '<div class="gsTgWapLkInnerMain"><canvas id="gsTgWapLkInnerCanvas"></canvas>';
        ...
        function setInnerSize(imginfo,isScling) {
            ...
            var $tar = $('#gsTgWapLkInnerV2'),
                $tarImg = $tar.find('.gsTgWapLkInnerMain'),
                $tarClose = $tar.find('.gsTgWapLkInnerClose'),
                imgTop = 0,
                ttp = $tar.offset().top;
            $tar.css({
            ...
            function animShow() {
                var drawJS = {
                    ...
                    updateDraw:function () {
                        tarRate = (showHeight - tarHeight)/($wHeight - tarHeight);
                        var tarTop = $tar.offset().top,
                            sclTop = this.tarStage.getBoundingClientRect().top;
                        imgTop = 0;
                        if(sclTop < $wHeight - tarHeight){
                            imgTop = $wHeight - tarHeight - sclTop;
                        }
                        imgTop = imgTop*tarRate;
                        var showY = tarHeight-showHeight + imgTop;
                        if(sclTop < 0){
                            showY = 0;
                        }
                        ctx.clearRect(0,0,tarWidth,tarHeight);
                        ctx.drawImage(ctxImg,0, showY,tarWidth,showHeight);
                    },
                    init:function () {
                        var func = this;
                        this.draw();
                ...
                var stage = drawJS.tarStage,
                    ctx = stage.getContext('2d'),ctxImg = new Image();
                ctxImg.src = tgImg;
